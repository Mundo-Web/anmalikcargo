...
const Home = ({ services = [], testimonies = [], faqs = [], generals = [], socials = [], staff_boss = null, benefits = [], indicators = [], landing = [], sliders = [], core_values = [] }) => {
    ...
    const getSocialIcon = (name) => {
        ...
        // Retorna el icono correspondiente o un icono por defecto
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            >
                ...
                <div className={`hidden lg:block bg-[#1a252f] transition-all duration-700 ease-in-out ${scrollY > 30 ? 'h-0 py-0 opacity-0 overflow-hidden' : 'h-auto py-2 opacity-100'
                    }`}>
            ...
            >
                        ...
                        {slidersToShow.map((slider, index) => (
                            <SwiperSlide key={index} className="relative">
                                <div className="absolute inset-0">
                                    ...
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/40 to-black/60 lg:bg-gradient-to-r lg:from-black/50 lg:via-black/30 lg:to-black/50"></div>
                                </div>
                            </SwiperSlide>
                        ))}
                    </Swiper>
                ...
                <div className="swiper-pagination-custom absolute bottom-4 mx-auto lg:bottom-6 left-1/2 transform  flex gap-1 z-20"></div>
                            ...
                            <div className="text-white space-y-4 lg:space-y-6 text-center lg:text-left animate-slideInLeft">
                                <h2 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight">
                                    {slidersToShow[currentSlide]?.name ? (
                                        slidersToShow[currentSlide].name.split(/(\*[^*]+\*)/g).map((part, index) => {
                                            if (/^\*[^*]+\*$/.test(part)) {
                                                // Palabra entre asteriscos
                                                return <span key={index} className="text-[#e5b437] animate-pulse">{part.replace(/\*/g, '')}</span>;
                                            }
                                ...
                                <div className='hidden lg:flex gap-4 pt-8'>
                                    <button
                                        className="swiper-button-prev-custom bg-[#e5b437] hover:bg-[#d4a332] text-white p-2 lg:p-3 rounded-full z-20 transition-all duration-300 hover:scale-110 hover:shadow-lg active:scale-95"
                                        aria-label="Previous slide"
                                    >
                                        <ChevronLeft className="w-4 h-4 lg:w-6 lg:h-6" />
                                    </button>
                                    <button
                                        className="swiper-button-next-custom bg-[#e5b437] hover:bg-[#d4a332] text-white p-2 lg:p-3 rounded-full z-20 transition-all duration-300 hover:scale-110 hover:shadow-lg active:scale-95"
